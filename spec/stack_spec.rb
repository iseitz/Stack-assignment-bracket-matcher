...
describe Stack do
  ...
  describe '#push' do
    it 'allows us to push in objects up to 20' do
      stack.push(:plate)
      stack.push('Fortune Cookie')
      stack.push(Time.now)
      stack.push(3.14159)
    end
    it 'raises an exception if stack is overflowed with more than 20 objects' do
      20.times do
        stack.push(:plate)
      end
      expect { stack.push('Fortune Cookie') }.to raise_error(StackOverflow)
    end
  end
